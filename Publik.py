...
╚══════════════════
"""
alevan="u19a6336222efe94acce1b05dd39c0940"
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment1":"lb ya kak ke #punyaa❂͜͡Ĝα₤αηĸ➣",
  #  "comment2":"Wkwkwk ＼（○＾ω＾○）／",
#    "comment3":"Lucu Banget!!! ヘ(^_^)ヘ",
#    "comment4":"Nice Kak (^_^)",
    "comment5":"Bot Auto Like ©By : ❂͜͡Ĝα₤αηĸ➣\nContact Owner Bot Kami: 👉line.me/ti/p/~azhura07_",    
    "commentOn":True,
    "commentBlack":{},
    "message":"Thx For Add Me (^_^)\nInvite Me To Your Group ヘ(^_^)ヘBy Ŧ€Äm ÄυŦĦōя bōŦ",    
    "blacklist":{},
    "wblacklist":False,
    "dblacklist":False,
    "Qr":{},
	 ...
	 if wait["Ghost"] == True:
		...
		random.choice(KAC).sendText(msg.to,"Itu Admin Kami (^_^)")	
              ...
              if msg.from_ in Creator:
                ...
                if targets == []:
                   ...
                   for target in targets:
                        try:
                            admin.append(target)
                            ale.sendText(msg.to,"Admin Ŧ€Äm ÄυŦĦōя bōŦ Ditambahkan")
                        except:
                            pass
              ...
              if msg.from_ in Creator:
                ...
                if targets == []:
                   ...
                   for target in targets:
                        try:
                            admin.remove(target)
                            ale.sendText(msg.to,"Admin Ŧ€Äm ÄυŦĦōя bōŦ Dihapus")
                        except:
                            pass
              ...
              if admin == []:
                  ...
                  mc = "╔═════════════════════════\n║        ADMIN Ŧ€Äm ÄυŦĦōя bōŦ\n╠═════════════════════════\n"
                  for mi_d in admin:
                      mc += "╠❂͜͡➣ " +ale.getContact(mi_d).displayName + "\n"
...
#--------------------------------------------------------
		...
		ale.sendText(msg.to,"Pasukan Absen!!")
                ale.sendText(msg.to,"Ŧ€Äm ÄυŦĦōя bōŦ1 Hadiir  \(ˆ▿ˆ)/")
                ale.sendText(msg.to,"Ŧ€Äm ÄυŦĦōя bōŦ2 Hadiir  \(ˆ▿ˆ)/")
                ale.sendText(msg.to,"Ŧ€Äm ÄυŦĦōя bōŦ3 Hadiir  \(ˆ▿ˆ)/")
                ale.sendText(msg.to,"Hadiir Semua Boss  \(ˆ▿ˆ)/")
